...
const StudentDashboard = ({ studentId }) => {
  ...
  return (
    <div className="container mx-auto px-4">
      {/* Book Appointment Card - CLICKABLE (gradient changed to honey/tomato) */}
      <div 
        onClick={() => navigate('/book-appointment')}
        className="bg-gradient-to-r from-honey-500 to-tomato-500 rounded-2xl p-8 mb-8 text-white shadow-xl cursor-pointer transform transition hover:scale-[1.02]"
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center gap-2 mt-4 text-honey-100">
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2">
          <Clock className="w-6 h-6 text-tomato-500" />
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <button 
              onClick={() => navigate('/book-appointment')}
              className="mt-4 px-4 py-2 bg-tomato-500 text-white rounded-lg hover:bg-tomato-600 transition"
            >
                ...
                <div className="flex justify-between items-start flex-wrap gap-2">
                  <div>
                    ...
                    <div className="flex items-center gap-4 mt-1 text-sm text-gray-600">
                      <span className="flex items-center gap-1"><Calendar className="w-3 h-3 text-tomato-400" /> {booking.day}</span>
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-tomato-400" /> {booking.time}</span>
                    </div>
